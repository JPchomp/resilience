function [dmat,flowmat,dualmat] = getfloodmat(from, to, distance, capacity,dir,T)
...
for s = sn:en
    ...
    if dir == 0
    ssn=sn;
    elseif dir == 1                                                         % This block checks that the network is undir, then it only calculates the upper diagonal
    ssn=s;
    end

    for t = ssn:en                                                          % If you start from s here we can calculate only the upper/lower diag and save time (Only for Undir Network).
