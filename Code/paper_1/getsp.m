function [nsp,csp] = getsp(from,to,weights,s,t) %ADD G input, call removedege, run SP %Input a column of sources and destinations.
...
nsp=zeros(length(s),20);                                        % Define here some length
...
for i = 1:length(s)
    ...
    nsp(i,end) = length(v1);                                   % embed the length of the shortest path at the "end" of the matrix
    end
end
    
%Now that i have the possibility of s(i) = t(i) delete those.
nsp(sum(nsp,2) == 0,:) = [];
csp(csp == 0) = [];
