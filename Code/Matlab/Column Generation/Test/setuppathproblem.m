...
for j = (1:length(pathcosts))
    for i = (1:length(vnodes(j,(1:vnodes(j,end))))-1)
        counter = counter+1;
        ...
        if vnodes(j,vnodes(j,end)) == vnodes(j,i+1)
...
length(pathcosts)/length(s);

kopath = [(1:length(s))' , repmat(eye(length(s)),1,length(pathcosts)/length(s))];
