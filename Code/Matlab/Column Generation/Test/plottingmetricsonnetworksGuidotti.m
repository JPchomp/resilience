...
dgp = DG;dgp(dgp == 0) = []; dgp = reshape(dgp,num-1,num);dgp = dgp'; SDG = sqrt(sum(((dgp-MDG).^2)/(length(dgp)),2));SDG = round(SDG,2);
%% One Link Out    
FTD2 = FTD; FTD2(1,:) = [];
G2 = graph(FTD2(:,1), FTD2(:,2) , FTD2(:,3));
DG2 = distances(G2);
MDG2 = sum(DG2,2)/(length(DG2)-1);
dgp = DG2;dgp(dgp == 0) = []; dgp = reshape(dgp,num-1,num);dgp = dgp'; SDG2 = sqrt(sum(((dgp-MDG2).^2)/(length(dgp)),2));SDG2 = round(SDG2,2);
%% 5 Links Out
FTD3 = FTD; FTD3([2,5,6,7,8],:)=[];
G3 = graph(FTD3(:,1), FTD3(:,2) , FTD3(:,3));
DG3 = distances(G3); DG3(DG3 == Inf) = 20;
MDG3 = sum(DG3,2)/(length(DG3)-1);
dgp = DG3; dgp(dgp == 0) = []; dgp = reshape(dgp,num-1,num);dgp = dgp'; SDG3 = sqrt(sum(((dgp-MDG3).^2)/(length(dgp)),2)); SDG3= round(SDG3,2);
%% Plotting    
...
set(gcf, 'Position',  posize);
set(gcf,'color',color);
        p2 = plot(G,'XData',xlocation,'YData',ylocation,...
        'EdgeLabel',distance,...
        'NodeLabel', SDG,...
        'LineWidth',1);...
