...
function [dmat,DS] = GSP_CG_Body_traffic_matrix_function(T,from,to,distance,capacity,FTCD,MFTCD,MV,MPC)
...
dualmat = zeros(length(from),1);
%Matrix where the flows will be stored
flowmat = zeros(length(from),1);
%DM = zeros(n,n,lT);
    ...
    for t = ssn:en                                                          % If you start from s here we can calculate only the upper/lower diag and save time (Only for Undirected Network).
        ...
        if i~=j 
            ...
            try
                ...
                [ M(i,j) , ~, DS{i,j} , ~ , ~, ~, ~, ~] = solveoptim( from, to, distance, capacity, T(i,j) ,[i],[j],capmatrix,FTCD,MFTCD,8,MV,MPC,0,1);
